...
public class ManageItemListAction(context: ServletContext, request: HttpServletRequest, response: HttpServletResponse, @inject("doma_config") val domaConfig: Config, val itemDao: ItemDao, val itemImageDao: ItemImageDao): HtmlAction(context, request, response) {
    companion object{
    ...
    override fun get() {
        ...
        responseFromTemplate("/manage/items", arrayOf({val itemList =  items}))
        logger.exit()
    }
}